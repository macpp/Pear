...
impl Spanned for CallPattern {
    fn span(&self) -> Span {
        match self.name {
            Some(ref name) => name.span().unstable().join(self.expr.span()).unwrap_or(Span::call_site()),
            None => self.expr.span()
        }
    }
}
...
impl Parse for Case {
    fn parse(input: SynParseStream) -> PResult<Self> {
        ...
        let span = case_span_start.join(input.cursor().span().unstable()).unwrap_or(Span::call_site());
